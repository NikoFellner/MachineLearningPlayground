...
class NeuralNetwork_2(nn.Module):
    ...
    def __init__(self, IMG_SIZE, NUM_EPOCHS, INPUT_CHANNELS=3, DEV="cpu", DYNAMIC_QUANT=False):
        ...
        self.acc_test = 0
        self.dev = DEV
        self.conv1 = nn.Conv2d(INPUT_CHANNELS, 16, (3, 3), padding=[1,1])
    ...
    def train_one_epoch(self, model):
        ...
        for i, data in enumerate(self.trainLoader):
            ...
            samples = samples.to(device=self.dev)
            labels = labels.type(torch.LongTensor).to(device=self.dev)
